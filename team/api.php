...
// Classe per gestione registrazioni
class RegistrationManager {
    ...
    public function createRegistration($data) {
        $sql = "
            ...
            (name, surname, email, phone, birth_date, category, experience, password_hash, bio, photo, status, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())
        ";
    ...
    public function getPendingRegistrations() {
        return $this->db->fetchAll("
            SELECT * FROM pilot_registrations 
            WHERE status = 'pending' 
            ORDER BY created_at DESC
        ");
    }
    
    public function approveRegistration($registrationId, $adminId) {
        // Ottieni dati registrazione
        $registration = $this->db->fetchOne("
            SELECT * FROM pilot_registrations WHERE id = ?
        ", [$registrationId]);
        ...
        $sql = "
            INSERT INTO pilots 
            (name, surname, email, phone, birth_date, category, bio, photo, 
             password_hash, experience, status, created_at, approved_by, approved_at) 
    ...
    public function rejectRegistration($registrationId, $reason, $adminId) {
        $this->db->query("
            UPDATE pilot_registrations 
            SET status = 'rejected', 
                rejected_at = NOW(), 
...
// Classe per autenticazione piloti
class PilotAuthManager {
    private $db;
    ...
    public function login($email, $password) {
        $pilot = $this->db->fetchOne("
            SELECT id, name, surname, email, password_hash, category, status 
            FROM pilots 
            WHERE email = ? AND status = 'active'
        ", [$email]);
        
        if ($pilot && password_verify($password, $pilot['password_hash'])) {
            session_start();
            $_SESSION['pilot_id'] = $pilot['id'];
            $_SESSION['pilot_name'] = $pilot['name'];
            $_SESSION['pilot_email'] = $pilot['email'];
    ...
    public function isLoggedIn() {
        session_start();
        return isset($_SESSION['pilot_id']);
    }
    
    public function getCurrentPilot() {
        if (!$this->isLoggedIn()) {
            return null;
        }
        
        return $this->db->fetchOne("
            SELECT id, name, surname, email, phone, birth_date, category, bio, photo 
            FROM pilots 
            WHERE id = ?
        ", [$_SESSION['pilot_id']]);
    }
